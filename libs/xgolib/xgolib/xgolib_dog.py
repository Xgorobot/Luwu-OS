...
def p_control(err=0, kp=1, val_a=5, val_b=20):
    tmp = err * kp
    if tmp >= val_b:
        tmp = val_b
    elif tmp <= val_a and tmp >= 0:
        tmp = val_a
    elif tmp <= 0 and tmp >= 0 - val_a:
        tmp = 0 - val_a
    elif tmp <= 0 - val_b:
        tmp = 0 - val_b
    return tmp
...
class XGO_DOG():
    ...
    def __init__(self, port, baud=115200, version="xgomini", verbose=False):
        ...
        if self.version[:2] == 'MW' or self.version[0] == 'W':
            changePara('xgomini')
        elif self.version[0] == 'M':
            changePara('xgomini')
    ...
    def arm_set_angle2(self, th2, th3):
        """设置机械臂2、3轴角度"""
        offset = self.arm_param["arm_angle_offset"]
        self.servo_set_angle(53, th2 + offset[1])
        self.servo_set_angle(52, th3 + offset[2])
        self.arm_param["arm_angle_lt"][1] = th2
        self.arm_param["arm_angle_lt"][2] = th3
    ...
    def turn_to(self, theta, vyaw=60, emax=4):
        cur_yaw = self.read_yaw()
        des_yaw = self.init_yaw + theta
        cnt = 0
        vel_turn = p_control(des_yaw - cur_yaw, 0.4, val_a=9, val_b=vyaw)
        while True:
            cur_yaw = self.read_yaw()
            if abs(cur_yaw - des_yaw) > 30:
                vel_turn = p_control(des_yaw - cur_yaw, 0.8, val_a=9, val_b=vyaw)
            else:
                vel_turn = p_control(des_yaw - cur_yaw, 0.2, val_a=8, val_b=vyaw)
            
            # print(vel_turn, des_yaw - cur_yaw)

            
            if abs(des_yaw - cur_yaw) < emax:
                self.turn(vel_turn)
                cnt = cnt + 1
                if cnt > 5:
                    break
    ...
    def read_motor(self):
        ...
        angle = []
        if self.__unpack():
            for i in range(min(self.rx_COUNT + 1, 15)):
                if i < 12:
    ...
    def arm_servo_angle(self, servo_id, angle):
        if servo_id < 1 or servo_id > 4:
            print("ERROR!Illegal Servo ID!")
            return
        try:
            angle_u8 = conver2u8(angle, XGOparam["ARM_LIMIT"][servo_id - 1])
        except:
            print("Error!Illegal Value!")
            return
        if servo_id == 1:
            XGOorder["ARM_SERVO_ANGLE_1"][1] = angle_u8
            self.__send("ARM_SERVO_ANGLE_1")
        elif servo_id == 2:
            XGOorder["ARM_SERVO_ANGLE_2"][1] = angle_u8
            self.__send("ARM_SERVO_ANGLE_2")
        elif servo_id == 3:
            XGOorder["ARM_SERVO_ANGLE_3"][1] = angle_u8
            self.__send("ARM_SERVO_ANGLE_3")
        else:
            XGOorder["ARM_SERVO_ANGLE_4"][1] = angle_u8
            self.__send("ARM_SERVO_ANGLE_4")
    ...
    def _init_servo_param(self):
        """内部方法：初始化舵机参数"""
        servo_idlt = [11,12,13,21,22,23,31,32,33,41,42,43,51,52,53,54,91,92,93,94,95,96,97,98,99]
        servo_reg = [80,81,82,83,84,85,86,87,88,89,90,91,113,93,94,112,161,162,163,164,165,166,167,168,169]
        param_map = [1,2,3,1,2,3,1,2,3,1,2,3,4,5,6,7,8,8,8,8,8,8,8,8,8]
        param_lt = [-70,50,-70,90,-30,30,-65,65,-50,83,-90,90,-175,175,-180,180]
        self.servo_param = [servo_idlt, servo_reg, param_map, param_lt]

    def arm_init(self, L1, L2, offset1=0, offset2=0):
        """初始化机械臂参数
        Args:
            L1: 机械臂第一节长度
            L2: 机械臂第二节长度
            offset1: 偏移参数1 (可选，默认0)
            offset2: 偏移参数2 (可选，默认0)
        """
        self.arm_param = {}
        self.arm_param["arm_len"] = [L1, L2]
        self.arm_param["arm_angle_offset"] = [0, offset1-3, offset2-11, 0]
        self.arm_param["arm_angle_lt"] = [0, 0, 0, 0]
        self.arm_param["arm_pos"] = [0, 0, 0, 0, 0]
        
    def _servo_get_param(self, idx):
        """内部方法：获取舵机参数"""
        if self.servo_param is None:
            return [0, 0, 0]
            
        servo_lt = self.servo_param[0]
        reg_servo = self.servo_param[1]
        param_map = self.servo_param[2]
        param_lt = self.servo_param[3]
        
        idx_pos = servo_lt.index(idx) if idx in servo_lt else -1
        if idx_pos == -1:
            return [0, 0, 0]
        
        reg = reg_servo[idx_pos]
        map_idx = param_map[idx_pos]
        pos_start = param_lt[(map_idx * 2) - 2]
        pos_end = param_lt[(map_idx * 2) - 1]
        
        return reg, pos_start, pos_end
    ...
    def arm_set_polar_base(self, rlen, z):
        """设置机械臂极坐标"""
        arm_len = self.arm_param["arm_len"]
        _L1 = arm_len[0]
        _L2 = arm_len[1]
        _th1 = self.arm_param["arm_angle_lt"][0]
        
        d_len = math.sqrt(rlen**2 + z**2)
        
        # 检查长度是否在有效范围内
        if d_len > (_L1 + _L2) or d_len < math.fabs(_L1 - _L2):
            return
        
        # 计算角度
        if d_len != (_L1 - _L2):
            alpha = math.acos((( _L1**2 + d_len**2 - _L2**2) / (2 * _L1 * d_len))) * 180 / math.pi
        else:
            alpha = 180
        
        beta = math.atan2(z, rlen) * 180 / math.pi
        _th2 = 90 - (alpha + beta)
        if d_len != (_L1 - _L2):
            gamma = math.acos((( _L1**2 + _L2**2 - d_len**2) / (2 * _L1 * _L2))) * 180 / math.pi
        else:
            gamma = 0
        _th3 = 90 - gamma

        # 设置角度
        self.arm_set_angle2(_th2, _th3)
        
        # 更新位置
        x = rlen * math.cos(_th1 * math.pi / 180)
        y = rlen * math.sin(_th1 * math.pi / 180)
        self.arm_param["arm_pos"] = [x, y, z, rlen, _th1]

    def arm_set_polar(self, rlen, z, runtime=-1):
        """带时间的极坐标设置"""
        if runtime == 0:
            self.arm_set_polar_base(rlen, z)
            return
        self.arm_pos_update()
        pos_now = self.arm_param["arm_pos"]
        if runtime < 0:
            d_l = rlen - pos_now[3]
            d_z = z - pos_now[2]
            runtime = math.sqrt(d_l**2 + d_z**2)  * 0.006

        if runtime != 0:
            tim_st = time.time()
            while time.time() - tim_st < runtime:
                t1 = map_val(time.time() - tim_st, 0, runtime, pos_now[3], rlen)
                t2 = map_val(time.time() - tim_st, 0, runtime, pos_now[2], z)
                self.arm_set_polar_base(t1, t2)
                time.sleep(0.005)
        self.arm_set_polar_base(rlen, z)

    def arm_pos_update(self):
        """更新手臂位置状态"""
        arm_angle = self.arm_param["arm_angle_lt"]
        th1, th2, th3 = arm_angle[0], arm_angle[1], arm_angle[2]
        arm_len = self.arm_param["arm_len"]
        _L1, _L2 = arm_len[0], arm_len[1]
        
        alpha = 90 - th2
        beta = (90 - alpha) + th3
        
        z = (_L1 * math.sin(alpha * math.pi / 180)) - (_L2 * math.sin(beta * math.pi / 180))
        rlen = (_L1 * math.cos(alpha * math.pi / 180)) + (_L2 * math.cos(beta * math.pi / 180))
        x = rlen * math.cos(th1 * math.pi / 180)
        y = rlen * math.sin(th1 * math.pi / 180)
        self.arm_param["arm_pos"] = [x, y, z, rlen, th1]

    def arm_set_polar_offset(self, dlen, dz, runtime=0):
        """极坐标偏移（带时间）"""
        if runtime > 0:
            self.arm_pos_update()
        pos = self.arm_param["arm_pos"]
        self.arm_set_polar(pos[3] + dlen, pos[2] + dz, runtime)

    # 机械臂抓手设置角度
    def arm_set_grab(self, angle, runtime=-1):
        """设置抓手角度"""
        offset = self.arm_param["arm_angle_offset"][3]
        a_now = self.arm_param["arm_angle_lt"][3]
        
        if runtime < 0:
            runtime = math.fabs(angle - a_now) * 0.01
        
        elif runtime != 0:
            tim_st = time.time()
            while time.time() - tim_st < runtime:
                t1 = map_val(time.time() - tim_st, 0, runtime, a_now, angle)
                self.servo_set_angle(51, t1 + offset)
                time.sleep(0.005)
        self.servo_set_angle(51, angle + offset)
        self.arm_param["arm_angle_lt"][3] = angle

    # 移动设置速度 时间
    def move_set_vel(self, vx=18, vy=0, vyaw=0, runtime=0):
        if vx == 0 and vy == 0 and vyaw == 0:
            self.move_x(0)
            self.move_y(0)
            self.turn(0)
            return
        self.move_x(vx+self.vel_x_offset)
        self.move_y(vy+self.vel_y_offset)
        self.turn(vyaw)
        if runtime > 0:
            time.sleep(runtime)
            self.move_x(0)
    ...
    def servo_set_angle(self, servo_id, angle):
        reg, pos_start, pos_end = self._servo_get_param(servo_id)
        XGOorder["SERVO_CTRL"][0] = reg
        if servo_id < 90:
            XGOorder["SERVO_CTRL"][1] = int(map_val(angle, pos_start, pos_end, 1, 255))
            self.__send("SERVO_CTRL")
        else:
            angle_mapped = int(map_val(angle, pos_start, pos_end, 1, 4095))
            XGOorder["SERVO_CTRL"][1] = int((angle_mapped >> 8) & 255)
            XGOorder["SERVO_CTRL"][2] = int(angle_mapped & 255)
            self.__send("SERVO_CTRL", len=2)

    # 机器狗姿态 重心 
    def posture_set_cog(self, x=0, y=0, z=98):
        # 设置重心位置
    ...
    # 机器狗姿态 重心 2
    def posture_set_cog2(self, x=0, y=0):
        # 设置重心位置
        self.translation("x",x)
        self.translation("y",y)

    # 机器狗姿态 角度
    def posture_set_euler(self, roll=0, pitch=0, yaw=0):
        self.attitude('r', roll)
        self.attitude('p', pitch)
        self.attitude('y', yaw)

    # 设置速度偏差
    def set_vel_offset(self, vel_x_offset=0, vel_y_offset=0):
        self.vel_x_offset = vel_x_offset
        self.vel_y_offset = vel_y_offset

    # 设置当前朝向为0度初始朝向
    def set_yaw_init(self):
        self.init_yaw = self.read_yaw()